  ...
  <div class="taskhall">
    ...
    <label-page>
      <div slot="banner-text">
        ...
        <p
          class="text"
        >临床指南是由临床药物遗传学实施联盟、（CPIC），荷兰皇家药剂师协会成立的药物遗传学工作组（DPWG），加拿大药物基因组学药品安全网（CPNDS），其他专业协会，中国专家共识基于PGx药物剂量发布的指南推荐，简要介绍了基于基因型的临床建议，并提供可下载的指南PDF文件。</p>
        <p class="text">我们欢迎已发布的有关PGx剂量准则的任何信息-请与我们联系。</p>
      </div>
      <div slot="filter-box">
        <p class="fr">共{{listAllNum}}条临床记录，当前显示{{(pageNum*pageSize-9)+"-"+pageNum*pageSize}}条</p>
        <div class="fl">
          ...
          <el-button class="filter-button" @click="getPageData()" v-if="!isTextInput">筛选</el-button>
        </div>
      </div>
      <div class="content-box">
        <p v-if="!tableData&&!loading" style="text-align:center; padding-top:20px;">暂无数据</p>
        <div class="table authority" v-loading="loading" style="min-height:150px;">
              ...
              >
                ...
                <td>
                  <p
                    ...
                    @click="toSearchContent('',item['geneName'],'gene','1',true)"
                  >
                    <el-tag color="#368ebe" class="el-tag">
                      <i class="el-icon-document"></i>
                      {{item["geneName"]}}
                    </el-tag>
                  </p>
